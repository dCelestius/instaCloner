...
export default function SchedulePage() {
    ...
    const handleSchedule = async () => {
        ...
        try {
            ...
            loadUpcomingPosts()
            alert("Batch submitted! Check results below and the Calendar preview.")
        } catch (e) {
    ...
    return (
        <div className="min-h-screen bg-black text-slate-50 flex flex-col pt-24 pb-32">
            <SiteHeader
                step={5}
                ...
                <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
                                                    ...
                                                    <button
                                                        ...
                                                        className={cn(
                                                            ...
                                                            selectedAccounts.has(acc.id)
                                                                ? "bg-emerald-500/20 border-emerald-500 text-emerald-300 shadow-[0_0_10px_rgba(16,185,129,0.15)]"
                                                                : "bg-zinc-950 border-white/5 hover:border-white/20 hover:bg-zinc-900"
                                                        )}
                                                    >
                                                        <div className="relative">
                                                            <img src={acc.thumb} className="w-8 h-8 rounded-full border border-white/10" alt="" />
                                                            {selectedAccounts.has(acc.id) && (
                                                        ...
                                                        <div className="flex-1 min-w-0">
                                                            ...
                                                            <div className="text-[10px] text-zinc-500 truncate capitalize">{acc.type}</div>
                                                        </div>
                                                    </button>
                                                ))}
                                            </div>
                    ...
                    {authStep === 'connected' && (
                        <div className="lg:col-span-8 space-y-4 animate-in fade-in slide-in-from-right-8 duration-500 delay-200 lg:sticky lg:top-24 h-fit max-h-[calc(100vh-8rem)] overflow-y-auto pr-1 no-scrollbar">
                            ...
                            <div className="bg-zinc-900/90 backdrop-blur border border-white/10 rounded-xl p-4 shadow-xl">
                                ...
                                {scheduleMode === 'manual' ? (
                                    <div className="animate-in fade-in slide-in-from-top-1">
                                        <div className="flex items-center gap-3">
                                            <Input
                                                ...
                                                className="bg-zinc-950 border-white/10 invert-calendar-icon text-xs h-8 w-fit" // Compact Input
                                            />
                                            <p className="text-[10px] text-zinc-500">
                                                *Sets fixed time for all
                                            </p>
                                        </div>
                                    </div>
                                ) : (
                                    <div className="space-y-4 animate-in fade-in slide-in-from-top-1">
                                        ...
                                        <div className="grid grid-cols-2 gap-4">
                                            ...
                                            <div className="space-y-1">
                                                <div className="flex justify-between items-center">
                                                    <Label className="text-[10px] text-zinc-400 font-medium">Spread Over</Label>
                                                    <span className="font-mono text-[10px] text-emerald-400">{scheduleDays} Days</span>
                                                </div>
                                                <Slider
                                                    value={[scheduleDays]}
                                                    min={1}
                                            ...
                                            <div className="space-y-1">
                                                <div className="flex justify-between items-center">
                                                    <Label className="text-[10px] text-zinc-400 font-medium">Min Gap</Label>
                                                    <span className="font-mono text-[10px] text-emerald-400">{minGapHours}h</span>
                                                </div>
                                                <Slider
                                                    value={[minGapHours]}
                                                    min={0.5}
                                            ...
                                            <div className="space-y-1 col-span-2">
                                                <div className="flex justify-between items-center">
                                                    <Label className="text-[10px] text-zinc-400 font-medium">Start Hour</Label>
                                                    <span className="font-mono text-[10px] text-emerald-400">{startHour}:00</span>
                                                </div>
                                                <Slider
                                                    value={[startHour]}
                                                    min={0}
                                            ...
                                            <div className="flex flex-col gap-2 col-span-2">
                                                <div className="flex items-center justify-between bg-zinc-950/50 p-1.5 rounded border border-white/5">
                                                    <Label className="text-[10px] text-zinc-400 cursor-pointer" onClick={() => setStartDayOption(prev => prev === 'today' ? 'tomorrow' : 'today')}>Start</Label>
                                                    <div className="flex text-[9px] font-bold bg-black rounded p-0.5 ml-2">
                                                        <button
                                                            onClick={() => setStartDayOption('today')}
                                                            className={cn("px-2 py-0.5 rounded transition-colors", startDayOption === 'today' ? "bg-zinc-800 text-white" : "text-zinc-600 hover:text-zinc-500")}
                                                        >Today</button>
                                                        <button
                                                            onClick={() => setStartDayOption('tomorrow')}
                                                            className={cn("px-2 py-0.5 rounded transition-colors", startDayOption === 'tomorrow' ? "bg-zinc-800 text-white" : "text-zinc-600 hover:text-zinc-500")}
                                                        >Tmrw</button>
                                                    </div>
                                                </div>

                                                <div className="flex items-center justify-between bg-zinc-950/50 p-1.5 rounded border border-white/5">
                                                    <Label className="text-[10px] text-zinc-400 cursor-pointer" onClick={() => setScheduleStrategy(prev => prev === 'fill' ? 'append' : 'fill')}>Strategy</Label>
                                                    <div className="flex text-[9px] font-bold bg-black rounded p-0.5 ml-2">
                                                        <button
                                                            onClick={() => setScheduleStrategy('fill')}
                                                            className={cn("px-2 py-0.5 rounded transition-colors", scheduleStrategy === 'fill' ? "bg-zinc-800 text-white" : "text-zinc-600 hover:text-zinc-500")}
                                                            title="Fill available gaps"
                                                        >Fill</button>
                                                        <button
                                                            onClick={() => setScheduleStrategy('append')}
                                                            className={cn("px-2 py-0.5 rounded transition-colors", scheduleStrategy === 'append' ? "bg-zinc-800 text-white" : "text-zinc-600 hover:text-zinc-500")}
                                        ...
                                        <div className="text-[9px] text-zinc-600 bg-black/20 border-t border-white/5 pt-2 flex justify-between">
                                            <span>
                                                {selectedReels.size} posts / {scheduleDays} days ≈ {Math.max(1, Math.round(selectedReels.size / scheduleDays))}/day
                                            </span>
                                            <span className="text-emerald-500/80">
                                                {scheduleStrategy === 'fill' ? `Fills gaps > ${minGapHours}h` : "Appends to end"}
                                            </span>
                                        </div>
                                    </div>
                                )}
